...
void FullScreen::Initialize(FullScreenSpriteCommon* FSSpriteCommon, SRVManager* srvManager, std::string textureFilePath)
{
	this->FSSpriteCommon_ = FSSpriteCommon;
...
void FullScreen::Draw(FullScreenSpriteCommon* FSSpriteCommon)
{
	this->FSSpriteCommon_ = FSSpriteCommon;
	FSSpriteCommon_->GetDx12Common()->GetCommandList().Get()->
		SetPipelineState(FSSpriteCommon_->GetGraphicsPipelineState().Get());
	FSSpriteCommon_->GetDx12Common()->GetCommandList().Get()->
		SetGraphicsRootSignature(FSSpriteCommon_->GetRootSignature().Get());
	FSSpriteCommon_->GetDx12Common()->GetCommandList().Get()->
...
ComPtr<ID3D12Resource> FullScreen::CreateBufferResource(size_t sizeInBytes)
{
	...
	hr = FSSpriteCommon_->GetDx12Common()->GetDevice().Get()->CreateCommittedResource(
